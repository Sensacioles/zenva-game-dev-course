...
var explosion_scene = preload("res://scenes/bullets/explosion.tscn") # Preload explosion scene
var current_tower:Tower # Store currently selected tower
var tower_menu:bool # Opened menu state
...
func _ready():
	...
	#$Towers/TowerBlaster.connect("shoot", create_bullet)
...
# Set selected tower, add crosshair if it's a mortar
...
func _input(event:InputEvent) -> void:
	...
	if place_tower:
		# Detect mouse motion and if tower menu is opened
		...
		# Detect if exit button is pressed and collapse all menus
